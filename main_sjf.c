...
int main()
{
    ...
    while (!sched_queue_empty() || active != NULL)
    {
        ...
        // Modified the next line to also print the PID of the currently active process
    ...
    {
        /*
         *  Modified next line to use PID of process, finish time (on clock), total wait time (in ready queue), and
         *  average wait time (time spent waiting in ready queue between active periods)
         */
...
int test_basics()
{
    struct Proc **p = calloc(3, sizeof(struct Proc *));
    struct Proc *q;
    struct Proc *r;
    int p0_pid;
    ...
    p[2] = proc_create(program_new(p3, 3));
    p0_pid = p[0]->pid;
    ...
    if (q->bc != 1 || q->duration != 10 || r->bc != 0 || r->duration != 4)
    {
    ...
    // Sanity check SJF priority calculation
    if (q->pid != p0_pid)
    {
        printf("Problem: IO-bound process not being given priority.\n");
        sched_free();
        free(p[0]);
        free(p[1]);
        free(p);
